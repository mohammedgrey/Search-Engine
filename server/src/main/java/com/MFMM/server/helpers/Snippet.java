    ...
    private String extendSnippet(String resultText,String[] wholeText,Integer startIndex, Integer numberOfElements)
    {
        if(startIndex != 0)
        {
            for (int i = startIndex - 1; i >= 0; i--) {
                ...
                if(resultText.length() >= STRING_MIN_LENGTH) return resultText;
            }
        }
        ...
        for (String word : wholeText) {
            ...
            if (isKeyWord)
            {
                ...
                Outloop: while(currentNormalizingWord.equals(matchQueue.peek()))
                {
                    ...
                    if(! currentNormalizingWord.equals(word) && processedKeyWords.get(currentNormalizingWord) == 1) break;
                    ...
                    processedKeyWords.put(currentNormalizingWord, processedKeyWords.get(currentNormalizingWord) - 1); //Decrement current word counter
            ...
            // (2) Inset to queue
            if(matchFound || isKeyWord) matchQueue.add(word);
        }

        // Check if the string is long enough
        if (result.length() < STRING_MIN_LENGTH) return extendSnippet(result,wholeText,startIndex,minSize);
